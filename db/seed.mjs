...
async function main() {
  ...
  for (const it of ITEMS) {
    await pool.query(
      ...
      [it.id, it.kind, it.name, it.description ?? null, it.icon ?? null, it.sellGold ?? 0]
    );
  }
  for (const eq of EQUIPMENT) {
    await pool.query(
      ...
      [eq.id, eq.domain, eq.slot, eq.name, eq.description ?? null, eq.icon ?? null,
  ...
  for (const rn of RUNES) {
    await pool.query(
      ...
      [rn.id, rn.name, rn.description ?? null, rn.icon ?? null, JSON.stringify(rn.effects), rn.maxCharges,
       rn.repairGold, rn.sellGold ?? 0]
    );
  }
